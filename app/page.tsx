...
function Video({episode, description, link, date}) {
    return (
	<div className="lg:flex lg:flex-row md:flex md:flex-col w-full p-5 rounded-lg  transition-all bg-white hover:shadow-xl hover:z-10 hover:-translate-y-3 cursor-pointer">
	    <Link href={link} target="blank" className="lg:flex lg:flex-row md:flex md:flex-col w-full">
	        <div className="flex flex-col mr-10 min-h-full justify-center items-center mr-10 lg:min-w-[40%] md:min-w-full">
	            <div className="lg:w-100 lg:h-60 md:w-60 bg-[#ff0e26] rounded-lg flex flex-col justify-center"><img src="/full.jpeg" width="100%" className="" /></div>
	        </div>
	        <div className="w-full h-full flex flex-col justify-center text-black lg:mt-0 mt-5">
...
function TopBar() {
    ...
    useEffect(() => {
	const text_container = document.getElementById("topbar-text-container");
	const circle_logo = document.getElementById("circle_logo");
	const navbar = document.getElementById("navbar");
	
	if (window.screen.width <= 576) {
	    text_container.classList.add("hidden");
	    circle_logo.addEventListener("click", () => {
    ...
    return (
	<nav className="shadow-xl lg:flex lg:flex-row md:flex md:flex-col items-center p-5 bg-[#f42f39] text-white z-10">
	    ...
	    <nav id="topbar-text-container" className="flex flex-row items-center gap-5">
	    ...
	    <nav id="navbar" className="z-20 absolute flex flex-col justify-start bg-[#f42f39] shadow-lg h-screen w-screen -left-250 top-20 p-5 transition-all lg:hidden">
	        ...
	        <Link className="w-45 tracking-[.25em] text-left text-white font-(family-name:--font-black) mt-3 mb-3" onClick={() => {nav_bar_opened = false; update_navbar(); }} href="/mp">Mundo Perdido...</Link>
	    </nav>
	</nav>
    );
}
...
function Footer() {
    ...
    return (
	<section id="footer" className="p-5 bg-[#f42f39] flex flex-col justify-center items-center mt-15">
...
function Content() {
	...
	if (window.screen.width <= 576) {
	    about_image.classList.add("hidden");
	} else {
	    if (about_image.classList.contains("hidden")) {
		about_image.classList.remove("hidden");
	    }
	...
	<section className="h-full overflow-auto">
	    <section className="bg-[#f42f39] h-full flex flex-col">
	        ...
	        <div className="h-full flex flex-row justify-center items-center">
	            <img src="/logo.jpeg" alt="logo" className="lg:w-[30%] md:w-[50%]" />
	        </div>
	    ...
	    <section id="about" className="h-auto lg:flex lg:flex-row md:flex md:flex-col overflow-hidden">
	         <div id="about-image" className="min-w-[50%] min-h-auto">
	            <img src="/about.jpeg" className="relative" />
	         </div>
	         <div className="p-5 pl-10 pr-10 flex flex-col justify-center lg:min-w-full lg:min-h-full md:min-w-screen md:min-h-auto bg-white shadow-xl shadow-x1/30 shadow-black z-10">
	             <p className="w-full tracking-[.20em] text-xl font-black mb-5 mt-5 font-(family-name:--font-black)">
	                 Sobre...
	             </p>
	             <p className="lg:w-[45%] text-sm/7 text-justify mb-2 mt-2">
	    ...
	    <section id="episodes" className="lg:p-20 h-auto">
